...
if(isset($_POST['submit'])) {
    ...
    if($opwd!='' && $npwd!='' && $cnpwd!='') {
        if($npwd===$cnpwd) {  
            $sql_query = "SELECT password as hashpwd FROM users WHERE id='$userid';";
            $result = mysqli_query($conn,$sql_query);
            $rows = mysqli_fetch_array($result);
            if(password_verify($opwd,$rows['hashpwd'])) {
                $hashpwd = password_hash($npwd,PASSWORD_BCRYPT);
                $sql_query = "UPDATE users SET password='$hashpwd' WHERE id='$userid';";
                $result = mysqli_query($conn,$sql_query);
                header("Location:./login.php");
            }
            else {
                $err_msg = "Old password entered did not match";
            }
        }
        else {
            $err_msg = "New passwords entered did not match";
        }
    }
    else {
        $err_msg ='Passwords not entered'; 
    }
}
?>
